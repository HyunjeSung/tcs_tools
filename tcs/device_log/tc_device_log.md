...
- 공통 전제 조건 충족
- 대상 log_item(예: `PMU_Monitoring`)이 최근 1 `logRowInterval` 이내에 최소 1회
  텔레메트리를 수신한 상태
...
- 공통 전제 조건 충족
- `logRowInterval`이 짧은 log_item(예: `PMU_Monitoring`, 60초) 선택 권장 — 시험시간 단축
...
## TC03 — 로그 타입별(prefix) 자동 수집 디렉토리 생성 확인

### 목적

`logpolicy.json`에 정의된 주기적(non-fault) `logItem`이, 해당 외부 디바이스로부터 실제
텔레메트리가 수신되는 경우 `/edge/log/device_log/<logItem>/`을 자동 생성하고 파일을
쓰는지 prefix(디바이스 그룹)별로 확인한다. Fault/FaultMon류(이벤트 기반 —
`PCS_Fault_P01/P02`, `BMS_Fault_P01/P02`, `BPU_Fault_P01/P02`, `PMU_Fault`,
`Advanced_HUB_Fault`, `PCS_Fault_Monitoring_Converter/Inverter_P01/P02`)는 주기적 로깅이
아니므로 본 TC 범위에서 제외한다.
...
4. `ls -d /edge/log/device_log/<logItem>` 로 실제 생성된 디렉토리 확인

### 기대 결과

| 항목 | 기준 |
|------|------|
| 디렉토리 | 텔레메트리가 도착 중인 logItem은 모두 `/edge/log/device_log/<logItem>/` 존재 |

### PASS/FAIL Criteria

| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC03-1 | EMSP 그룹 디렉토리 생성 (EMSP_Maintenance, EMSP_Installation) | boolean | true | `ls -d /edge/log/device_log/EMSP_Maintenance /edge/log/device_log/EMSP_Installation` |
| TC03-2 | PMU 그룹 디렉토리 생성 (PMU_Monitoring) | boolean | true | `ls -d /edge/log/device_log/PMU_Monitoring` |
| TC03-3 | MI 그룹 디렉토리 생성 (MI_Monitoring, MI_Device_Info) | boolean | true | `ls -d /edge/log/device_log/MI_Monitoring /edge/log/device_log/MI_Device_Info` |
| TC03-4 | PCS(CAN) 그룹 디렉토리 생성 (PCS_CAN_Monitoring_P01/P02) | boolean | true | `ls -d /edge/log/device_log/PCS_CAN_Monitoring_P01 /edge/log/device_log/PCS_CAN_Monitoring_P02` |
| TC03-5 | Advanced HUB 그룹 디렉토리 생성 (Advanced_HUB_Monitoring) | boolean | true | `ls -d /edge/log/device_log/Advanced_HUB_Monitoring` |
| TC03-6 | Meter 그룹 디렉토리 생성 (Meter) | boolean | true | `ls -d /edge/log/device_log/Meter` |
| TC03-7 | BMS 그룹 디렉토리 생성 (Operation/LifeCycle/Monitoring_P01/P02, 6개) | boolean | true | `ls -d /edge/log/device_log/BMS_Operation_P01 /edge/log/device_log/BMS_Operation_P02 /edge/log/device_log/BMS_LifeCycle_P01 /edge/log/device_log/BMS_LifeCycle_P02 /edge/log/device_log/BMS_Monitoring_P01 /edge/log/device_log/BMS_Monitoring_P02` |
| TC03-8 | BPU 그룹 디렉토리 생성 (BPU_CAN_Monitoring_P01/P02) | boolean | true | `ls -d /edge/log/device_log/BPU_CAN_Monitoring_P01 /edge/log/device_log/BPU_CAN_Monitoring_P02` |
| TC03-9 | C_Box 그룹 디렉토리 생성 (C_Box_Monitoring) | boolean | true | `ls -d /edge/log/device_log/C_Box_Monitoring` |
...
> 물리적으로 연결되어 있지 않아 도착 안 됨 확인됨 — 실물 연결 또는 MQTT 시뮬레이터로
> 텔레메트리를 직접 publish해야 실행 가능.
...
| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC07-1 | 재부팅 전후 파일명 동일 | boolean | true | `[ "$name_before" = "$name_after" ]` |
...
- 공통 전제 조건 충족 + TC07과 동일 대상 log_item

### 절차

1. TC07 절차로 재부팅 전 파일에 데이터 행이 최소 1개 존재함을 확인
...
| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC08-1 | 재부팅 경계에 빈 줄 존재 | boolean | true | `sed -n "${boundary_line}p" "$csv" \| grep -qE '^$'` |
| TC08-2 | 헤더 라인 유지 | boolean | true | `head -1 "$csv" \| diff - <(echo "$header_before")` |

---

## TC09 — 외부 디바이스 연결 해제/재연결 시 빈 행 삽입 (§AGSRS-548 미구현 확인)
...
| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC09-1 | 연결 해제 구간 빈 행 존재 (요구사항 AC 기준, 예상 FAIL) | boolean | true | `sed -n "${boundary_line}p" "$csv" \| grep -qE '^$'` |
| TC09-2 | device_connection 알림 수신 확인(참고용) | boolean | true | `journalctl -u docker-loader --no-pager \| grep -q "CAN connection state"` |
...
> 발생(TC08과 동일 코드 경로)하며, 연결 해제/재연결과는 무관하다. 즉 §AGSRS-548은
> 알림 수신 경로만 있고 실제 동작은 구현되어 있지 않음 — Jira 재확인/개발 이슈 등록
> 대상으로 보고할 것을 권장한다.

> **주의 (Flag, 정정 — 2026-08-21 실측):** TC09-2는 기본 설정에서 100% FAIL이
...
## TC10 — Archive 파일 개수(fileCount) 초과 시 FIFO 삭제
...
| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC10-1 | 파일 수가 수정된 fileCount(5) 이하 | boolean | true | `[ "$(ls "$archive_dir" \| grep -c '\.csv\.xz$')" -le 5 ]` |
| TC10-2 | 가장 오래된 파일 삭제됨 | boolean | true | `[ ! -f "$oldest_file" ]` |
| TC10-3 | fileCount 원복 확인 | boolean | true | `grep -A2 "\"logItem\": \"$log_item\"" logpolicy.json \| grep '"fileCount"' \| grep -q ": *$original_count"` |

---

## TC11 — Archive 파일 보관기간(retentionTime) 초과 삭제
...
1. 네트워크를 차단한 뒤 `get_log_data`로 앱이 직접 archive로 옮긴(정식으로
   `pushArchFileInfo()`에 등록된) 실제 파일 1개를 확보
2. 그 파일에 `touch -d`로 mtime을 `retentionTime` 초과 시점으로 강제 설정
...
| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC11-1 | 만료 파일 삭제 확인 | boolean | true | `[ ! -f "$expired_file" ]` |

> **주의 (Flag, 정정 — 2026-08-21 실측):** 최초 절차(디스크에 직접 더미 파일 생성)는
> `CloudUploadManager::arch_dir_files_` 인메모리 큐가 부팅 시 1회만
> (`enumerateExistingFilesInDirectory()`, `cloud_upload_manager.cpp:107`) 채워진다는
> 점을 놓쳐, `deleteArchFileBasedOnTime()`(`:1701-1705`, 인메모리 큐만 순회)이
> 더미 파일의 존재 자체를 모르는 상태가 되어 **어떤 대기시간을 줘도 삭제되지
> 않는다**(코드 결함이 아니라 절차상 앱의 추적 밖에 있는 파일을 대상으로 삼은
> 것이 원인). 앱이 정식 경로로 옮긴 실제 파일을 대상으로 삼도록 절차를 정정했다.

---

## SID0204 — Cloud upload and deletion

## TC12 — 업로드 성공/실패 journal 기록
...
1. 임의 log_item 파일을 toupload로 이동시켜 업로드 유발
...
| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC12-1 | 성공/실패 로그 중 하나 기록 | boolean | true | `journalctl -u docker-loader --no-pager \| grep -qE "Upload success for log item|Upload fail for log item"` |

---

## TC13 — 다수 로그 파일 Azure 업로드 시도 확인
...
> 최대 300초를 기다려야 `[Director]` 로그에 등장한다 — TC12/TC13 셸 검증 시
...
1. 대상 log_item 1개를 선정해 파일 3개 이상을 `toupload/`로 이동
...
| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC13-1 | 3개 파일 각각 업로드 시도 로그 확인 | boolean | true | `for f in "${files[@]}"; do journalctl -u docker-loader --no-pager \| grep '\[Director\]' \| grep -q "$f" \|\| exit 1; done` |

---

## TC14 — 네트워크 끊김 시 업로드 미시도 및 archive 누적
...
1. 현재 네트워크 상태 확인 (온라인이면 위 사전조건대로 차단, 오프라인이면 skip)
2. 로그 생성 → `forced_log_upload` IPC로 root에 `.csv.xz` 생성
3. idle thread 사이클(5초) 대기
4. `toupload/`, `archive/` 두 디렉토리를 모두 조회해 신규 파일이 실제로 어느 쪽으로
   이동했는지 확인
...
| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC14-1 | 신규 파일 이동 위치 확인 | boolean | archive | `if [ -f "$toupload_dir/$fname" ]; then echo toupload; elif [ -f "$archive_dir/$fname" ]; then echo archive; else echo none; fi` |

---

## SID0205 — Handling network disruption

## TC15 — 네트워크 중단 시 업로드 동작(root→archive 이동)

### 목적 (요구사항 §AGSRS-537)

네트워크 연결 상태에서 파일이 toupload로 이동한 후 네트워크가 차단되면, 이후 root에서
새로 압축된 파일들이 archive로 이동하는지 확인한다.
...
1. 네트워크 연결 상태에서 파일 1개를 toupload로 이동시켜 둠 (`isToUploadDirEmpty()`가
   false가 되도록)
2. 네트워크 차단
3. 신규 로그 생성 → root 파일 발생
4. idle thread 사이클 대기 후 `archive/` 이동 확인

### 기대 결과

| 항목 | 기준 |
|------|------|
| archive 이동 | 신규 root 파일이 archive로 이동 |

### PASS/FAIL Criteria

| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC15-1 | archive 이동 확인 (device_log 책임 범위) | boolean | true | `[ -f "$archive_dir/$(basename "$xz")" ]` |
| TC15-2 | cloud_broker 10회 재시도 확인(참고용, 아래 Flag 참고) | boolean | true | `journalctl -u docker-loader --no-pager \| grep -q '\[Director\] Upload failed\. retry'` |

> **주의 (Flag, 정정):** device_log(`cloud_upload_manager.cpp`) 자체에는 고정 횟수(10회)
> 재시도 카운터가 없다 — `isInternetAvailable()`이 false면 즉시 archive로 이동시키고
> 이후 `uploadIdleThread()`의 5초 주기 무한 루프로 toupload 이동만 재시도한다(횟수 제한
> 없음). **"업로드 10회 재시도"는 device_log가 아니라 toupload 이후 실제 업로드를
> 수행하는 `cloud_broker`(로그 태그 `CB`)의 `BlobUploadDirector`에 구현되어 있다** —
> `blob_upload_director.cpp:26`의 `kRetrySec{5,10,20,40,80,160,320,640,1280,2560}`(정확히
> 10개 값, 지수 백오프)와 `retry_or_delete()`(`:233`)가 그 로직이다. `BlobUploadDirector`는
> device_log와 동일한 `/edge/log/toupload`를 스캔하므로(`blob_upload_director.hpp:85`
> `kUploadRoot`) device_log가 만든 파일도 여기서 재시도된다. 10회 초과 시 파일을
> 삭제하고 `NOTI_FILE_UPLOAD_RESULT`(result=fail)를 device_log로 돌려보내는데, 이것이
> TC12의 "Upload fail for log item" 로그가 실제로 발생하는 경로다. 따라서 "10회 재시도"
> 자체의 검증은 device_log 단독 TC가 아니라 cloud_broker 쪽 TC 명세에서 다뤄야 하며,
> 본 TC(TC15-1)는 device_log 책임 범위(root→archive 이동)만 검증한다.

---

## TC16 — 네트워크 복구 시 자동 업로드 재개
...
- 공통 전제 조건 충족, TC14 또는 TC15로 archive에 파일이 누적된 상태

### 절차

1. archive에 파일이 1개 이상 누적된 상태 확인
2. 네트워크 복구
3. `internet_status` notification 수신 후(또는 다음 폴링) `archive/` → `toupload/`
   이동 확인
...
| 항목 | 기준 |
|------|------|
| toupload 이동 | archive의 파일이 toupload로 이동, Azure 업로드 결과 확인(TC12/13 연계) |

### PASS/FAIL Criteria

| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC16-1 | archive→toupload 이동 확인 | boolean | true | `[ ! -f "$archive_dir/$fname" ] && [ -f "$toupload_dir/$fname" ]` |

---

## TC17 — 로컬 파일(archive) 업로드 주기적 재시도

### 목적 (요구사항 §AGSRS-540)

네트워크가 연결된 상태에서 archive에 남은 파일들이 idle thread의 주기적 사이클에서
계속 toupload로 재시도되는지 확인한다 (`archiveToUpload()` —
`cloud_upload_manager.cpp:847`).

### 사전 조건

- 공통 전제 조건 충족, `logcount.json`의 대상 log_item `perDayArchive` > 0

### 절차

1. archive에 파일 존재 상태에서 `perDayRoot=0`, `perDayArchive>0`으로 설정(테스트용)
2. 네트워크 연결 유지 상태에서 idle thread 사이클(5초) 반복 대기
3. archive → toupload 이동 및 Azure 업로드 결과 확인

### 기대 결과

| 항목 | 기준 |
|------|------|
| 이동 | archive 파일이 toupload로 이동 후 사라짐 |

### PASS/FAIL Criteria

| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC17-1 | archive 파일이 이동됨 | boolean | true | `[ ! -f "$archive_dir/$fname" ]` |
...
| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC18-1 | 재부팅 후 toupload 파일 처리됨 | boolean | true | `journalctl -b 0 -u docker-loader \| grep -q "Upload success"` |

---

## SID0206 — Upload with data traffic calculation

## TC19 — 1일 root 폴더 업로드 개수 제한
...
4. 23:50(자정 루틴, TC21) 도달 시 `perDayRoot`가 `defaultPerDay`로 초기화되는지 확인
...
| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC19-1 | toupload 미이동 | boolean | true | `[ ! -f "$toupload_dir/$fname" ]` |
| TC19-2 | archive 이동 확인 | boolean | true | `[ -f "$archive_dir/$fname" ]` |

---

## TC20 — 1일 archive 폴더 업로드 개수 제한
...
2. 로그 생성 → root 파일 발생 → archive로 이동 확인(TC19와 동일 경로)
...
| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC20-1 | toupload 미이동 유지 | boolean | true | `[ ! -f "$toupload_dir/$fname" ]` |

> **주의 (Flag, 정정 — 2026-08-21 실측, TC16/TC17/TC19/TC20 공통):** 두 가지를
> 놓쳐 이전 실행에서 TC16-1/TC17-1/TC19-1이 FAIL했다(TC20은 아래 2번의 측정
...
>    TC10(logpolicy.json/fileCount)과 동일하게 `docker-loader` 재시작이 필요하다.
>    TC16은 또한 `perDayArchive`가 기본 0(자정 롤오버 전에는 항상 0,
...
>    우회하는 것이 설계 의도다. TC19/TC20처럼 perDayRoot=0의 효과를 검증하려면
...
>    유도해야 한다. TC24 절차(위 34-40행 주석)가 이미 쓰던 기법과 동일하다.

---

## TC21 — 자정 루틴: perDayRoot 초기화 및 잔여 quota archive 이월
...
| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC21-1 | perDayArchive 이월 계산 일치 | boolean | true | `[ "$archive_after" -eq "$((archive_before + root_before))" ]` |
| TC21-2 | perDayRoot 초기화 | boolean | true | `[ "$root_after" -eq "$default_per_day" ]` |

---

## TC22 — 월 전환 시 로그 카운트 초기화
...
| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC22-1 | perDayRoot=defaultPerDay | boolean | true | `[ "$root_after" -eq 10 ]` |
| TC22-2 | perDayArchive=0 | boolean | true | `[ "$archive_after" -eq 0 ]` |

---

## TC23 — 재부팅 후 업로드 설정 파일(logcount.json) 유지
...
| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC23-1 | 값 유지 확인 | boolean | true | `[ "$root_before" -eq "$root_after" ] && [ "$archive_before" -eq "$archive_after" ]` |

---

## TC24 — 라운드로빈 기반 균등 업로드 선택 확인
...
2. idle thread 사이클(5초 주기)을 **logpolicy.json 전체 log_type 수(대상 log_item
   수가 아님, 이 벤치 기준 32개 확인) × 2바퀴 이상** 반복 대기하며, 각 log_item의
   `toupload/<logItem>/` 파일 수 증가를 매 사이클 폴링해 어느 log_item이 선택됐는지
   기록
3. 대상 log_item별 선택 횟수를 집계

> **주의 (Flag, 정정 — 2026-08-21 실측):** `selectNextRootLogType()`은 대상으로 고른
> 5개 log_item만 도는 게 아니라 `log_type_keys_`(logpolicy.json 전체 loggingRules,
> 이 DUT에서 32개) 전체를 라운드로빈한다. "대상 log_item 수 × 3회"(예: 5×3=15사이클
> =75초)로는 전체 한 바퀴(32×5초=160초)도 못 돌아, 대상 중 일부가 관측 창 안에서
> 아예 선택되지 못한 것을 "편중"으로 오판한다. 대기 시간은 반드시 전체 log_type
> 수 기준으로 산정할 것. 또한 archive/root→toupload 이동은 `isToUploadDirEmpty
> (log_type)`도 함께 요구하므로(cloud_upload_manager.cpp:826,860), 앞선 TC가 특정
> log_item의 toupload에 미처리 파일을 남겨둔 채 이 TC를 이어서 실행하면 라운드로빈
> 선택 자체는 일어나도 toupload 카운트에는 반영되지 않을 수 있다 — 가능하면
> `--tc24` 단독 실행 권장.
...
| 항목 | 기준 |
|------|------|
| 선택 분포 | 특정 log_item에 편중되지 않고 대상 log_item 간 선택 횟수 편차가 1 이내 (라운드로빈) |

### PASS/FAIL Criteria

| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC24-1 | log_item별 선택 횟수 편차 ≤1 | boolean | true | `max=$(printf '%s\n' "${counts[@]}" \| sort -n \| tail -1); min=$(printf '%s\n' "${counts[@]}" \| sort -n \| head -1); [ $((max-min)) -le 1 ]` |
...
## TC25 — 웹 강제 업로드 (forced_log_upload IPC)
...
| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC25-1 | IPC 응답 OK | boolean | true | `echo "$resp" \| grep -q '"error_code":0'` |
| TC25-2 | toupload에 신규 파일 생성 | boolean | true | `[ "$(ls "$toupload_dir" | wc -l)" -gt "$before_count" ]` |

---

## SID0208 — Mass Production Feature (EOL)

## TC26 — EOL 로그 생성 (1초/1분 개별 파일)
...
1. `mosquitto_pub`으로 `SERVICE_SET_FACTORY_EOL_MODE`(`set_factory_eol_mode`)
   `{"eol_mode": true}` 발행
2. 2~5분 대기하며 텔레메트리 publish 지속
3. `/edge/log/eol/eol_1sec.csv`, `eol_1min.csv` 각각의 행 수와 마지막 행 시각 확인
4. `set_factory_eol_mode` `{"eol_mode": false}` 발행하여 종료
...
| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC26-1 | eol_1sec.csv 행 증가 | boolean | true | `[ "$(wc -l < eol_1sec.csv)" -gt "$rows_before" ]` |
| TC26-2 | eol_1min.csv 행 증가 | boolean | true | `[ "$(wc -l < eol_1min.csv)" -gt "$rows_before" ]` |
...
> 근거 수집 추가함). §AGSRS-491의 "AC 미구현"(TC27/TC28) 범위를 넘어서는, EOL
...
## TC27 — EOL 로그 압축 형식 (zip vs xz, §AGSRS-491 AC 미구현 확인)
...
- TC26과 동일, EOL 로그가 rotation/compress 되는 시점까지 대기 가능

### 절차

1. TC26 절차로 EOL 로그(`eol_1sec.csv`/`eol_1min.csv`)가 생성되도록 유도
...
| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC27-1 | zip 압축 파일 존재 확인 (요구사항 AC 기준, 예상 FAIL) | boolean | true | `[ "$(find /edge/log/eol -name '*.zip' \| wc -l)" -gt 0 ]` |
| TC27-2 | 실제로는 xz만 생성됨(참고용) | boolean | true | `[ "$(find /edge/log/eol -name '*.xz' \| wc -l)" -gt 0 ]` |
...
## TC28 — Factory EOL Mode ON 시 Field logging 중단 여부 (§AGSRS-491 AC 미구현 확인)
...
| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC28-1 | EOL 모드 중 field 로깅 중단 확인 (요구사항 AC 기준, 예상 FAIL) | boolean | true | `[ "$(wc -l < "$csv")" -eq "$rows_before" ]` |
| TC28-2 | 실제로는 필드 로깅 계속됨(참고용) | boolean | true | `[ "$(wc -l < "$csv")" -gt "$rows_before" ]` |
...
## TC29 — Factory Reset 시 전체 로그 삭제 및 EOL 모드 해제
...
| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC29-1 | eol 디렉토리 삭제 | boolean | true | `[ ! -d /edge/log/eol ]` |
| TC29-2 | device_log 디렉토리 삭제 | boolean | true | `[ ! -d /edge/log/device_log ]` |
| TC29-3 | toupload/device_log 디렉토리 삭제 | boolean | true | `[ ! -d /edge/log/toupload/device_log ]` |
| TC29-4 | IPC 응답 OK | boolean | true | `echo "$resp" \| grep -q '"error_code":0'` |
| TC29-5 | 재부팅 후 필드 로깅 재개 확인 | boolean | true | `[ "$(wc -l < "$csv")" -gt "$rows_before_reboot" ]` |
| TC29-6 | 재부팅 후 EOL 모드 기본 비활성 확인 | boolean | true | `[ ! -f /edge/log/eol/eol_1sec.csv ] || [ "$(wc -l < /edge/log/eol/eol_1sec.csv)" -eq "$eol_rows_before_reboot" ]` |
...
> 보인다(양산 라인에서 리셋 후 전원 차단을 가정). TC29-5(재부팅 후 필드 로깅 재개)는
> 반드시 재부팅을 포함해서 판정할 것 — 재부팅 생략 시 오탐(false fail) 가능. TC29-6은
...
| 기준 ID | 설명 | 타입 | 기준값 | 셸 검증 |
|---------|------|------|--------|---------|
| TC30-1 | 추출 후 폴더 이동 확인 | manual | — | 개발 완료 후 확정 (아래 Flag 참고) |
...
🟢 **A** (TC30만 IPC 미구현으로 실행 불가 — 나머지는 TC09/TC27/TC28 포함 boolean 스크립트로 무인 실행 가능. 단 TC09/TC27/TC28는 요구사항 미구현이 코드로 확정되어 실행 시 FAIL이 예상됨)

| TC | 등급 | 비고 |
|----|------|------|
| TC01~TC26, TC27~TC29 | A | 무인 실행 가능 (TC09/TC27/TC28는 Flag 항목 — boolean 스크립트로 실행은 가능하나 요구사항 미구현으로 FAIL 예상) |
| TC30 | B | EOL 로그 추출 IPC 자체가 미구현(Jira도 "개발 중" 명시) — 트리거할 대상이 없어 실행 불가, 구현 완료 후 활성화 대상 |
...
| TC07 | AGSRS-546 + `log_policy_manager.cpp:1028-1048`(Case A: existing_file 재사용) | High |
| TC08 | AGSRS-547 + `log_policy_manager.cpp:1103-1137`(writeFileHeaderWithColumnsName Case A 빈 행) | High |
| TC09 | AGSRS-548 + `device_log.cpp:311`(NOTI_DEVICE_CONNECTION 구독), `device_log.cpp:639-644`(handle_noti_device_connection, DEBUG 로그만 남기고 no-op) | **Flag — 미구현 확정. 알림 수신 경로는 있으나 CSV 조치 없음(빈 행 삽입은 재부팅 재사용 경로에서만 발생, TC08과 동일). DUT 실측 없이 FAIL 확정적** |
| TC10 | AGSRS-28 AC(FIFO) + AGSRS-511 + `cloud_upload_manager.cpp:1615`(deleteArchFileBasedOnCount) | High |
| TC11 | AGSRS-545 + `cloud_upload_manager.cpp:1676`(deleteArchFileBasedOnTime), `logpolicy.json:retentionTime` | High |
| TC12 | AGSRS-538 + `cloud_upload_manager.cpp:241`(handleFileUploadResult) | High |
| TC13 | AGSRS-512 + `cloud_upload_manager.cpp:133`(createMetaFile, uploadYearMonth), `cloud_broker/blob_upload_director.cpp`(kUploadRoot=/edge/log/toupload 스캔, 실제 업로드 시도 주체) | High |
| TC14 | AGSRS-536 + `cloud_upload_manager.cpp:816`(rootToArchiveOrToUpload) | High |
| TC15 | AGSRS-537 + `cloud_upload_manager.cpp:816`(root→archive 이동), `cloud_broker/blob_upload_director.cpp:26,233`(kRetrySec 10개, 실제 10회 재시도는 여기 구현) | High(경로 이동) / **Flag(정정 — "10회 재시도"는 device_log가 아닌 cloud_broker 책임으로 확인됨)** |
| TC16 | AGSRS-513 + `cloud_upload_manager.cpp:449`(cloud_connected_action) | High |
| TC17 | AGSRS-540 + `cloud_upload_manager.cpp:847`(archiveToUpload) | High |
| TC18 | AGSRS-539 + `cloud_upload_manager.cpp:96`(init/enumerateExistingFilesInDirectory) | Medium (재기동 시나리오는 코드상 추정, 실측 필요) |
| TC19 | AGSRS-514 + `cloud_upload_manager.cpp:1338`(isUploadAllowed), `logcount.json:perDayRoot` | High |
| TC20 | AGSRS-543 + `cloud_upload_manager.cpp:1338`(isUploadAllowed archive 분기) | High |
| TC21 | AGSRS-514(자정 부분) + AGSRS-544 + `cloud_upload_manager.cpp:1409`(updateLogUploadLimitsDaily) | High |
| TC22 | AGSRS-542 + `cloud_upload_manager.cpp:1432`(updateLogUploadLimitsMonthly) — 코드 값(root=default,archive=0)이 Jira 기대결과와 완전 일치 | High |
| TC23 | AGSRS-541 + `cloud_upload_manager.cpp`(loadUploadInfoMap/writeToLogCountJson 계열) | Medium (재부팅 유지 로직은 파일 영속성 기반 추정) |
| TC24 | AGSRS-498 AC1(우선순위 업로드) + `cloud_upload_manager.cpp:1310`(selectNextRootLogType 라운드로빈) | **Flag — 요구사항은 우선순위, 코드는 라운드로빈. 검증 목표를 라운드로빈 균등성으로 재설정(사용자 확인)** |
| TC25 | AGSRS-515 + `device_log.cpp:463`(handle_request_forced_log_upload), `cloud_upload_manager.cpp:314`(handleForcedLogUploadRequest) | High |
| TC26 | AGSRS-516 + `eol_logger.cpp:155`(processEolLogging), `eolpolicy.json`(eol_1min, eol_1sec — eol_10min은 정책에서 삭제됨) | High (단, IPC 필드명 `eol_mode` vs 요구사항 `factory_eol_mode` 표기 차이 있음) |
| TC27 | AGSRS-491 AC("zip format") + `log_compress.cpp`(compressToXz만 존재, zip 미발견) | **Flag — 미구현 확정. DUT 실측 없이 FAIL 확정적** |
| TC28 | AGSRS-491 AC("Field logging is disabled in EOL mode") + `device_log.cpp:455-461`(stopLogging 미호출), `device_log.cpp:425`(stopLogging 유일 호출부는 factory_reset뿐) | **Flag — 미구현 확정. DUT 실측 없이 FAIL 확정적** |
| TC29 | AGSRS-491 AC(리셋 시 전체 삭제/EOL 해제) + `device_log.cpp:417-443`(handle_request_factory_reset) | High (단, 재부팅 필요 전제는 Medium) |
| TC30 | AGSRS-549 + 코드 전역 검색(핸들러 미발견) | **Flag — 요구사항 자체가 미구현 명시, 코드도 미구현 확인** |
